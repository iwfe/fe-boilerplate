...
/*自动添加版本号*/
let gulpManifest = require('../plugins/manifestGulpPlugin.js');
/**
 * 处理用户输入
 */
let _InputEnv = {
...
let plugins=function(){
    ...
    if(versionType==0){
        return through.obj(function(file, encoding, resolver) {
            console.log(chalk.yellow(`you are using 自动添加版本号的方式 上传`));
            resolver(null,file)
        })
    }
};
    ...
    return new Promise(function(resolve) {
        gulp.src(__dirname)
            ...
            .pipe(confirm({
                ...
                proceed: function (answer) {
                    if(versionType==1){
                        console.log(chalk.yellow(`您的${Project}在${_InputEnv.env}环境的新版本号为${_InputEnv.version}`));
                    }
                    if (answer == 'y') {
                        var param = {
                            ...
                            formData: {
                                ...
                                isCoverVer: 1,
                                file: fs.createReadStream(uploadDistFolder + path.sep + `${Project}.zip`)
                            }
                        };
