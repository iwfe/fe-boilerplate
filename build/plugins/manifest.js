 ...
 * 递归文件处理
 */
WebpackAssetsManifest.prototype.flatternDir = function(dirArr)
{
    var results = []
    ...
    if(extractJsCss){
      for(let key in _manifests){
        ...
        if(path.extname(filename)=='.js' || path.extname(filename)=='.css'){
            transArgs.versionFiles.push(filename);
        }
      }
    }
    if(!!versionDir){ //递归需要加版本号的文件夹，添加进去
       transArgs.versionFiles = transArgs.versionFiles.concat(this.flatternDir(versionDir));
    }
    transArgs.versionFiles.forEach(function(item) {
