...
Plugin.prototype.touchstart = function(e) {
    ...
    self.touchtimer = Number(new Date());
    self.touchstartx = touches.pageX;
    self.touchstarty = touches.pageY;
...
Plugin.prototype.touchmove = function(e) {
    ...
    // 移动容器
    $sliderControl.css({
        '-webkit-transform': 'translate3d(' + (touches.pageX - self.touchstartx) + 'px,0,0)'
    });
...
Plugin.prototype.touchend = function(e) {
    ...
    if (touch.pageX > self.touchstartx + sliderWidth * 0.2 || touch.pageX > self.touchstartx + sliderWidth * 0.1 && (Number(new Date()) - self.touchtimer < 250)) {
        self.direction = 'previous';
        self.slide();
    } else if (touch.pageX < self.touchstartx - sliderWidth * 0.2 || touch.pageX < self.touchstartx - sliderWidth * 0.1 && (Number(new Date()) - self.touchtimer < 250)) {
        self.direction = 'next';
        self.slide();
    } else {
        // 归位
        $sliderControl[0].style[transform] = "translateX(0px)";
        $sliderControl[0].style[duration] = 500;
...
// 逐个显示隐藏分页
Plugin.prototype.hidePagination = function() {
    var self = this,
        $sliderControl,
        $desc,
        $sliderPage;
...
/**
 * [到指定的图片]
 * @param  {[type]} index [指定图片位置，或者下一张：next,上一张：prev]
 * @return {[type]}       [description]
 */
...
// 具体动画逻辑,index为第几张图片
Plugin.prototype.slide = function(index) {
    var self = this,
        // 当前下标
        currentIndex,
        value,
...
//隐藏左右按钮
Plugin.prototype.slideSwitch = function(index) {
    var self = this;
    var $element = self.$element;
    var options = self.options;
    if (!options.isLoop) {
...
// Plugin.prototype.css3slide = function(){
//     var self = this;
...
Plugin.prototype.loadContent = function(index) {
...
Plugin.prototype.generateLayout = function() {
    var html = '',
        self = this,
        index,
        items,
...
/**
 * 设置页码导航高亮
 * @param {[type]} index [页码下标]
 */
Plugin.prototype.setActive = function(index) {
    var self = this,
        $element,
        currentIndex,
        sliderLen,
...
// 生成页面导航／左右箭头
Plugin.prototype.generateStuff = function() {
    var self = this,
        $element,
        $prevButton,
        $nextButton,
...
Plugin.prototype.setOptions = function(o) {
